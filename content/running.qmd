...
```{.random-table}
caption: The Other’s secret means
columns: [d6, means]
rows:
  - [1, 'Money, lots of it']
...
```{.random-table}
caption: Weaknesses of the Other
columns: [d6, weakness]
rows:
...
```{.random-table}
caption: The Other’s real goals
columns: [d6, goal]
rows:
...
```{.random-table}
caption: In case of…
columns: [1d26, incident]
rows:
  - [1.1, Attack]
  - [1.2, Departure or resignation]
  - [1.3, Disappearance]
  - [1.4, Elimination]
  - [1.5, Espionage]
  - [1.6, Extraction]
...
```{.random-table}
caption: The reaction is…
columns: [1d26, reaction]
rows:
  - [1.1, Hand over to the authorities]
  - [1.2, Corrupt]
  - [1.3, Destroy credibility]
  - [1.4, Eliminate]
  - [1.5, Cover up]
  - [1.6, Intercept]
  - [2.1, Leave it alone or ignore]
  - [2.2, Manipulating opinion]
  - [2.3, Threatening]
  - [2.4, Entrapment]
  - [2.5, Bringing to court]
  - [2.6, Surveillance]
```
...
```{.random-table}
caption: Faction motivations
columns: [d6, motivation]
rows:
  - [1, Greed]
  - [2, Lust]
  - [3, Hatred]
...
```{.random-table}
caption: What is the attitude of the client?
columns: [d6, attitude]
rows:
...
```{.random-table}
caption: "What is the client's position?"
columns: [d6, position]
rows:
...
```{.random-table}
caption: "What is the client's motivation?"
columns: [d6, motivation]
rows:
...
```{.random-table}
caption: Internal life or a few minutes at the coffee machine
columns: [1d66, event]
rows:
...
```{.random-table}
caption: Attitudes of witnesses
columns: [1d26, attitude]
rows:
...
```{.random-table}
caption: Example of how dangerous the events are
columns: [d6, result]
rows:
...
```{.random-table}
caption: "Adventures: The Yacht"
columns: [d6, adventure]
rows:
...
```{.random-table}
caption: Rumours/events
columns: [d66, rumours/events, keywords]
rows:
...
```{.random-table}
caption: Trigger and request
columns: [d66, trigger, request]
rows:
  ...
  - ["2.3", "Suddenly, with direct involvement", "Intercepting sensitive informatio"]
...
```{.random-table}
caption: Departments and services
columns: [d666, departments and services]
rows:
...
```{.random-table}
caption: Job and position
columns: [d26, job, position]
rows:
...
```{.random-table}
caption: Attitude and nature
columns: [d26, attitude, nature]
rows:
...
```{.random-table}
caption: Location and office stories
columns: [d26, location, office stories]
rows:
...
```{.random-table}
caption: Gang relationship
columns: [d6, relationship]
rows:
...
```{.random-table}
caption: Gang colours (roll two or three times)
columns: [d26, colour]
rows:
  - ["1.1", Silver]
  - ["1.2", White]
...
```{.random-table}
caption: Gang-related locations
columns: [d26, location]
rows:
  - ["1.1", Sweatshop]
  - ["1.2", Sleazy brothel]
